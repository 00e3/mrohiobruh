...
void render::init( ) {
	...
	log = Font( XOR( "Lucida Console" ), 10, FW_DONTCARE, FONTFLAG_DROPSHADOW );
	...
	arrow = Font( XOR( "Acta Symbols W95 Arrows" ), 16, FW_NORMAL, FONTFLAG_ANTIALIAS );
	nap = Font( XOR( "Tahoma" ), 12, FW_NORMAL, FONTFLAG_DROPSHADOW );
}
...
void render::rect( vec2_t pos, vec2_t dim, Color color ) {
	g_csgo.m_surface->DrawSetColor( color );
	g_csgo.m_surface->DrawOutlinedRect( pos.x, pos.y, pos.x + dim.x, pos.y + dim.y );
}

void render::rect_filled( vec2_t pos, vec2_t dim, Color color ) {
	g_csgo.m_surface->DrawSetColor( color );
	g_csgo.m_surface->DrawFilledRect( pos.x, pos.y, pos.x + dim.x, pos.y + dim.y );
}

void render::rect_filled_fade( vec2_t pos, vec2_t dim, Color color, int a1, int a2 ) {
	g_csgo.m_surface->DrawSetColor( color );
	g_csgo.m_surface->DrawFilledRectFade( pos.x, pos.y, pos.x + dim.x, pos.y + dim.y, a1, a2, false );
}

void render::rect_outlined( vec2_t pos, vec2_t dim, Color color, Color color2 ) {
	rect( pos.x, pos.y, dim.x, dim.y, color );
	rect( pos.x - 1, pos.y - 1, dim.x + 2, dim.y + 2, color2 );
	rect( pos.x + 1, pos.y + 1, dim.x - 2, dim.x - 2, color2 );
}
...
void render::flat_sphere( vec3_t origin, float radius, float angle, float scale, Color color ) {
	static std::vector< Vertex > vertices{};
	...
	for ( float lat{}; lat < ( math::pi * scale ); lat += step ) {
		...
		for ( float lon{}; lon < math::pi_2; lon += step ) {
			vec3_t point{
				origin.x + ( radius * std::sin( lat ) * std::cos( lon ) ),
				origin.y + ( radius * std::sin( lat ) * std::sin( lon ) ),
				origin.z + ( 0.2f * radius * std::cos( lat ) ) //  (e.g., 0.5)
			};

			vec2_t screen;
			if ( render::WorldToScreen( point, screen ) )
				vertices.emplace_back( screen );
		}
