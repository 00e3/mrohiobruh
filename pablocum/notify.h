...
public:
	__forceinline NotifyText(const std::string& text, Color color, float time) : m_text{ text }, m_color{ color }, m_time{ time }, m_max_time{ time } {}
};
...
public:
	...
	void think() {
		int		x{ 8 }, y{ 5 }, size{ render::log.m_size.m_height + 1 };
		Color	color;
		float	left;
		...
		for (size_t i{}; i < m_notify_text.size(); ++i) {
			...
			render::log.string(x, y, color, notify->m_text);
			y += size;
		}
	}
};
