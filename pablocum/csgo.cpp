...
bool CSGO::init( ) {
	...
	sv_maxunlag->m_flags |= FCVAR_REPLICATED;
	...
	m_hud_chat = m_hud->FindElement<CHudChat>( XOR( "CHudChat" ) );
	...
	m_eventListeners = pattern::find( m_client_dll, XOR( "8B 35 ? ? ? ? 0F 1F 80 ? ? ? ? 39 14 86 74 ? 40 3B C1 7C ? 8D 45 ? 50 51 B9" ) ).add( 0x2 ).get< CUtlVector<void*>* >( );
