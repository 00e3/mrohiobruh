...
inline float DistanceToRay( const vec3_t& pos, const vec3_t& rayStart, const vec3_t& rayEnd, float* along = NULL, vec3_t* pointOnRay = NULL ) {
	...
	const float& rangeAlong = dir.dot( pos - rayStart );
...
static bool TraceToExit( vec3_t start, vec3_t dir, vec3_t& end, CGameTrace& trEnter, CGameTrace& trExit, float flStepSize, float flMaxDistance )
{
	...
	while ( flDistance <= flMaxDistance )
	{
		...
		const vec3_t vecTrEnd = end - ( flStepSize * dir );
		const int& nCurrentContents = g_csgo.m_engine_trace->GetPointContentsWorld( end, CS_MASK_SHOOT | CONTENTS_HITBOX );
		...
		if ( ( nCurrentContents & CS_MASK_SHOOT ) == 0 || ( ( nCurrentContents & CONTENTS_HITBOX ) && nStartContents != nCurrentContents ) )
		{
			...
			if ( trExit.m_startsolid && ( trExit.m_surface.m_flags & SURF_HITBOX ) )
			{
				...
				if ( trExit.hit( ) && !trExit.m_startsolid )
				{
					end = trExit.m_endpos;
					return true;
				}
			}
			else if ( trExit.hit( ) && !trExit.m_startsolid )
			{
				...
				else if ( !bExitIsNodraw || ( bStartIsNodraw && bExitIsNodraw ) ) // exit nodraw is only valid if our entrace is also nodraw
				{
					const vec3_t& vecNormal = trExit.m_plane.m_normal;
					if ( dir.dot( vecNormal ) <= 1.0f )
					{
...
bool penetration::run( PenetrationInput_t* in, PenetrationOutput_t* out ) {
	...
	if ( in->m_from == g_cl.m_local ) {
		...
		Weapon* weapon = in->m_from->GetActiveWeapon( );
		if ( !weapon )
			return false;
	...
	// weapon damage.
	out->m_damage = weapon_info->m_damage;

	// get damage and hitgroup for our shot.
	out->m_hitgroup = in->m_from->FireBullet( in->m_start,
		angAimDir,
	...
	if ( in->m_target ) {
		if ( !in->m_can_pen && iPenCount != 4 )
			return false;

		if ( out->m_hitgroup > HITGROUP_GENERIC ) {
			if ( iPenCount != 4 )
	...
	float& fCurrentDamage )
{
	...
	static CGameTrace exitTr;
	exitTr.clear( );
	if ( !TraceToExit( tr.m_endpos, vecDir, penetrationEnd, tr, exitTr, 4, 90.f ) )
	{
		// ended in solid
		if ( ( g_csgo.m_engine_trace->GetPointContentsWorld( tr.m_endpos, CS_MASK_SHOOT ) & CS_MASK_SHOOT ) == 0 )
		{
			return true;
		}
	}
	...
	else
	{
		...
		flPenetrationModifier = ( flPenetrationModifier + pExitSurfaceData->m_game.m_penetration_modifier ) / 2.f;
		flDamageModifier = ( flDamageModifier + pExitSurfaceData->m_game.m_damage_modifier ) / 2.f;
	}
	...
	float flTotalLostDamage = ( fCurrentDamage * flDamLostPercent ) + MAX( 0.f, ( 3.f / flPenetrationPower ) * 1.25f ) * ( flPenMod * 3.f ) + ( ( flPenMod * ( flTraceDistance * flTraceDistance ) ) / 24.f );
