	...
	for (int i{ 1 }; i <= g_csgo.m_entlist->GetHighestEntityIndex(); ++i) {
		...
		if (g_menu.main.skins.knife.get() > 0 && ent->is(HASH("CPredictedViewModel"))) {
			...
			case KNIFE_BUTTERFLY:
				...
				weapon->m_nFallbackStatTrak() = g_menu.main.skins.stattrak_butterfly.get() ? 1338 : -1;
			...
			if (g_menu.main.skins.custom_paintkit.get())
			{
				for (auto i = 0; i < map_head->last_element; ++i)
				{
					...
					paint_kit->color0 = g_menu.main.skins.color.get();
					paint_kit->color1 = g_menu.main.skins.color1.get();
					paint_kit->color2 = g_menu.main.skins.color2.get();
					paint_kit->color3 = g_menu.main.skins.color3.get();
				}
			}
...
void Skins::UpdateItem(Weapon* item) {
	...
	CHudElement* SFWeaponSelection = g_csgo.m_hud->FindElement<CHudElement>(XOR("SFWeaponSelection"));
