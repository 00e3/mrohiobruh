...
public:
	...
	__forceinline float& m_flSpawnTimeGrenade( ) {
		return get< float >( 0x29b0 );
	}
...
public:
	...
	__forceinline void InvalidateAllPhysics( )
	{
		InvalidatePhysicsRecursive( InvalidatePhysicsBits_t::ANGLES_CHANGED );
		InvalidatePhysicsRecursive( InvalidatePhysicsBits_t::ANIMATION_CHANGED );
		InvalidatePhysicsRecursive( InvalidatePhysicsBits_t::POSITION_CHANGED );
		InvalidatePhysicsRecursive( InvalidatePhysicsBits_t::VELOCITY_CHANGED );
	}
...
public:
	...
	float max_abs_delta( )
	{
		float flAimMatrixWidthRange = math::Lerp( std::clamp( m_speed_as_portion_of_walk_top_speed, 0.f, 1.f ), 1.0f, math::Lerp( m_aim_matrix_transition, 0.8f, 0.5f ) );

		if ( m_anim_duck_amount > 0 )
		{
			flAimMatrixWidthRange = math::Lerp( m_anim_duck_amount * std::clamp( m_speed_as_portion_of_crouch_top_speed, 0.f, 1.f ), flAimMatrixWidthRange, 0.5f );
		}

		return 58.f * flAimMatrixWidthRange;
	}
...
public:
	...
	mutable int        m_nframe_unlock_counter;
...
struct rebuilt_animstate_t {
	...
	int m_wpn_select, m_old_wpn_select;
	...
	void reset( ) {
		...
		m_deploy_rate_limiting = false;

		m_buttons = m_old_buttons = 0;

		m_landing = false;
		m_landing_multiplier = 0.f;
		...
		m_speed_portion_run =
			m_speed_portion_walk =
			m_speed_portion_crouch = 0.f;

		m_jumping = false;
		m_time_in_air = 0.f;

		m_strafe_cycle = m_strafe_weight = m_strafe_duration = 0.f;
	...
	void correct( CCSGOPlayerAnimState* state ) {
		...
		state->m_duration_in_air = m_time_in_air;
...
{
public:
	__forceinline CUtlVector< ClientHitVerify_T >& m_ImpactList( )
	{
		return *( CUtlVector< ClientHitVerify_T >* )( ( uintptr_t )this + 0xBA84 );
	}
	...
	{
		return get< vec3_t >( g_entoffsets.m_vecAbsOrigin );
	}
	...
	__forceinline ang_t& m_angAbsAngles( )
	{
		return get< ang_t >( g_entoffsets.m_angAbsAngles );
	}
	...
	{
		return get< int >( g_entoffsets.m_nComputedLODframe );
	}

	__forceinline float& m_flGuardianTooFarDistFrac( )
	{
		return get< float >( g_entoffsets.m_flGuardianTooFarDistFrac );
	}
	...
	__forceinline float& m_flGravity( )
	{
		// .text:10381AB3 00C    F3 0F 10 49 10             movss   xmm1, dword ptr [ecx+10h] ; Move Scalar Single-FP
		// .text:10381AB8 00C    F3 0F 5C 88 90 A2 00 00    subss   xmm1, dword ptr[ eax + 0A290h ]; Scalar Single - FP Subtract
		return get< float >( g_entoffsets.m_flGravity );
	}
	...
	{
		return get< CPlayerState >( g_entoffsets.pl );
	}
...
public:
	...
	void SetAbsAngles( const ang_t& angles, bool invalidate )
	{
		//static AbsTransFn set_abs_angles_fn = (AbsTransFn)(g_pUtils.FindPatternIDA("client.dll", "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8"));
		if ( invalidate )
		{
			InvalidatePhysicsRecursive( ANGLES_CHANGED );
			m_iEFlags( ) &= ~EFL_DIRTY_ABSTRANSFORM; // dont call calcabsoluteposition
		}

		math::AngleMatrix( angles, m_vecAbsOrigin( ), m_rgflCoordinateFrame( ) );

		m_angAbsAngles( ) = angles.normalized();
		m_angAbsRotation( ) = angles;
	}

	void SetAbsOrigin( const vec3_t& position, bool invalidate )
	{
		//	static AbsTransFn set_position_fn = (AbsTransFn)(g_pUtils.FindPatternIDA("client.dll", "55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8 ? ? ? ? 8B 7D"));
		if ( invalidate )
		{
		...
		math::AngleMatrix( m_angAbsAngles( ), position, m_rgflCoordinateFrame( ) );

		m_vecAbsOrigin( ) = position;
		m_vecOrigin( ) = position;
	}

	void SetAbsVelocity( const  vec3_t& velocity, bool invalidate )
	{
		//	static AbsTransFn set_velocity_fn = (AbsTransFn)(g_pUtils.FindPatternIDA("client.dll", "55 8B EC 83 E4 F8 83 EC 0C 53 56 57 8B 7D 08 8B F1 F3"));
		if ( invalidate )
		{
			InvalidatePhysicsRecursive( VELOCITY_CHANGED );
			m_iEFlags( ) &= ~EFL_DIRTY_ABSVELOCITY; // dont call CalcAbsoluteVelocity
		}
	...
	__forceinline float get_layer_seq_cycle_rate( CStudioHdr* hdr, std::ptrdiff_t seq )
	{
		using fn_t = float( __thiscall* )( decltype( this ), CStudioHdr*, std::ptrdiff_t );

		return ( *reinterpret_cast< fn_t** >( this ) )[ 216 ]( this, hdr, seq );
	}
	...
	__forceinline int LookupBone( const char* name )
	{
	...
	{
		const CBoneCache& cache = m_BoneCache( );
		if ( !cache.m_pCachedBones )
			return { 0, 0, 0 };

		if ( !matrix )
			matrix = cache.m_pCachedBones;
		...
		const mstudiohitboxset_t* set = hdr->GetHitboxSet( m_nHitboxSet( ) );
		if ( !set )
			return { 0, 0, 0 };
		...
		math::VectorTransform( center, matrix[ bbox->m_bone ], center );
	...
	__forceinline void GetBones( BoneArray* matrix )
	{
		const CBoneCache& cache = m_BoneCache( );
		if ( !cache.m_pCachedBones )
			return;

		std::memcpy( matrix, cache.m_pCachedBones, sizeof( BoneArray ) * cache.m_CachedBoneCount );
	}

	__forceinline void SetBones( BoneArray* matrix )
	{
		CBoneCache& cache = m_BoneCache( );
		if ( !cache.m_pCachedBones )
			return;

		std::memcpy( cache.m_pCachedBones, matrix, sizeof( BoneArray ) * cache.m_CachedBoneCount );
	}
	...
	__forceinline bool enemy( Player* from )
	{
		...
		if ( g_csgo.mp_teammates_are_enemies->GetInt( ) )
			return true;

		if ( !this || !from )
			return false;

		return m_iTeamNum( ) != from->m_iTeamNum( );
	}
