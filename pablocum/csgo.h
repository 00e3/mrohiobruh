...
struct UserData {
	...
	UserData(const CHAR* hwid, const int& uid, const std::string& name) {
		std::memcpy(m_hwid, hwid, sizeof(CHAR) * 39);
		m_uid = uid;
		m_name = name;
	}
};
...
public:
	...
	CUtlVector<void*>*		 m_eventListeners;
...
public:
	...
	/* inline this for it to be harder to find */
	__forceinline const bool& is_valid_hwid() {
		return true;

		return m_user_data.m_uid >= UserNameId::BREAKER && m_user_data.m_uid < UserNameId::USER_MAX;
	}
...
namespace game {
	__forceinline float GetClientInterpAmount() {
		const float& updaterate = std::clamp(g_csgo.cl_updaterate->GetFloat(), g_csgo.sv_minupdaterate->GetFloat(), g_csgo.sv_maxupdaterate->GetFloat());
		const float& lerp_ratio = std::clamp(g_csgo.cl_interp_ratio->GetFloat(), g_csgo.sv_client_min_interp_ratio->GetFloat(), g_csgo.sv_client_max_interp_ratio->GetFloat());

		return std::max(lerp_ratio / updaterate, g_csgo.cl_interp->GetFloat());
	}
